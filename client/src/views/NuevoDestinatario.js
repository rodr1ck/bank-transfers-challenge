...
const NuevoDestinatario = () => {
  ...
  const createDestinatario = (destinatario) => {
    ...
    axios
      .post("/api/destinatario/new", destinatario)
      .then((res) => {
  ...
  return (
    <div class="card">
      <div class="card-header">Nuevo Destinatario</div>
      <div class="card-body">
    <h5 class="card-title">{errors.map((err, index) => (
        <div className="alert alert-danger" role="alert">
          {err}
        </div>
      ))}</h5>
      <NuevoDestinatarioForm
        onSubmitProp={createDestinatario}
        iRut=""
        iName=""
        iEmail=""
        iPhone=""
